...
// Modified from original source by ODF 2012
...
// <ODF> Trying out a custom writer.
template <class Graph>
class custom_writer {
    typename property_map<Graph, edge_weight_t>::type typedef PMap;
public:
    custom_writer(PMap prop) : prop_(prop) {}
    template <class VertexOrEdge>
    void operator()(std::ostream& out, const VertexOrEdge& v) const {
        out << "[weight=\"" << get(prop_, v) << "\"]";
    }
private:
    PMap prop_;
};
...
int main(int,char*[])
{
  ...
  boost::write_graphviz(std::cout, g,
                        ...
                        custom_writer<Graph>(trans_delay),
